 ...
 * `read-all/route.ts`). Actor resuelto por header X-Actor-Id (temporal, D-29 de `users`, mismo
 * patron que `points`/`phasecontracts`/`support`). Autoservicio estricto: nunca la bandeja de otro. */
@RestController
...
public class NotificacionController {
    ...
    @GetMapping
    public NotificacionesBandejaResponse listar(@RequestHeader("X-Actor-Id") String actorId) {
        return NotificacionesBandejaResponse.from(listarNotificacionesUseCase.listar(UserId.of(actorId)));
    }

    @PutMapping("/{id}/read")
    public MarcarLeidaResponse marcarLeida(@RequestHeader("X-Actor-Id") String actorId, @PathVariable Long id) {
        return MarcarLeidaResponse.from(marcarLeidaUseCase.marcarLeida(UserId.of(actorId), id));
    }

    @PutMapping("/read-all")
    public MarcarTodasLeidasResponse marcarTodasLeidas(@RequestHeader("X-Actor-Id") String actorId) {
        return new MarcarTodasLeidasResponse(marcarTodasLeidasUseCase.marcarTodas(UserId.of(actorId)));
    }
}
