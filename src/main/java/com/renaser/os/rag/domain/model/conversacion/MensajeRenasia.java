 ...
 *
 * <p><b>D-102:</b> cada mensaje pertenece a UN {@link AgenteConversacional}. La conversacion
 * sigue siendo una sola fila por usuario (`conversaciones_renasia`), pero el historial se lee y
 * se escribe por agente: lo que la persona hablo con el tutor de cursos nunca aparece en el chat
 * del acompanante, ni viceversa. Es obligatorio en todas las fabricas a proposito — un mensaje
 * "sin agente" seria justo la mezcla que el dueno pidio no hacer.
 *
...
public final class MensajeRenasia {
    ...
    private final AgenteConversacional agente;
    ...
    public static MensajeRenasia escribirDeUsuario(MensajeRenasiaId id, UserId usuarioId, AgenteConversacional agente,
                                                    String contenido, Instant ahora) {
        return crear(id, usuarioId, agente, RolMensaje.USUARIO, contenido, List.of(), ahora);
    }

    /** El {@code id} entra por parametro: lo arma el caso de uso con el puerto {@code IdGenerator}. */
    public static MensajeRenasia escribirDeAsistente(MensajeRenasiaId id, UserId usuarioId,
                                                       AgenteConversacional agente, String contenido,
                                                       List<FuenteMensaje> fuentes, Instant ahora) {
        return crear(id, usuarioId, agente, RolMensaje.ASISTENTE, contenido, fuentes, ahora);
    }

    private static MensajeRenasia crear(MensajeRenasiaId id, UserId usuarioId, AgenteConversacional agente,
                                         RolMensaje rol, String contenido, List<FuenteMensaje> fuentes,
                                         Instant ahora) {
        Objects.requireNonNull(id, "id es obligatorio");
        requireUsuarioId(usuarioId);
        requireAgente(agente);
        requireContenido(contenido);
        requireFuentesSoloDeAsistente(rol, fuentes);
        return new MensajeRenasia(id, usuarioId, agente, rol, contenido, List.copyOf(fuentes), ahora);
    }

    /** Solo para el adaptador de persistencia. */
    public static MensajeRenasia rehydrate(MensajeRenasiaId id, UserId usuarioId, AgenteConversacional agente,
                                            RolMensaje rol, String contenido, List<FuenteMensaje> fuentes,
                                            Instant creadoEn) {
        requireAgente(agente);
        requireFuentesSoloDeAsistente(rol, fuentes);
        return new MensajeRenasia(id, usuarioId, agente, rol, contenido, List.copyOf(fuentes), creadoEn);
    }
    ...
    private static void requireAgente(AgenteConversacional agente) {
        if (agente == null) {
            throw new IllegalArgumentException("Todo mensaje pertenece a un agente (D-102): agente es obligatorio");
        }
    }
    ...
    public String toString() {
        ...
        return "MensajeRenasia[" + id + ", agente=" + agente + ", rol=" + rol + "]";
    }
}
