...
- `INVALID_EVIDENCE_PENALTY_POINTS = 10` (`evidence-ai/service.ts:21`), con el comentario "Same magnitude as Santuario's break penalty" — coincidía con `habits.SesionBloqueo.PENALIZACION_ROTURA_PUNTOS` en este backend, confirmando el valor. (**Corregido el 2026-09-22:** esa constante ya no existe — romper el Santuario dejó de descontar puntos. `Evidencia.PENALIZACION_EVIDENCIA_INVALIDA_PUNTOS` sobrevive porque solo se usa para **devolver** en `INVALID_EVIDENCE_REVOKED`; nadie descuenta.)
