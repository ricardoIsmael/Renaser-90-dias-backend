...
> **Esto describe el backend VIEJO, no el actual.** Se deja tal cual porque es la fuente de la que
> se portó la regla. La fórmula que corre hoy vive en `PuntajeGeneral` y **desde el 2026-09-22 es
> `75% hábitos + 25% cursos`**: las rocas (hoy OBJETIVOS) salieron del ranking general porque su
> porcentaje **es** la coherencia, que ya se muestra aparte en Hoy (D-128) y ordena el ranking de
> célula. Ver D-144 en `docs/MODULOS_A_AVANZAR.md` §8. También cambió el "sin dato → 100" que se
> lee abajo: desde D-131 el módulo sin dato **sale del promedio**, no vale 100.
...
- **Q-3 — ~~¿La fórmula GENERAL (50% hábitos + 35% rocas + 15% cursos) se mantiene igual cuando `habits`/`rocks`/`academy` existan, o se revisa?~~ RESUELTA el 2026-09-22 (D-144): se revisó.** El ranking general pasó a **75% hábitos + 25% cursos** y las rocas salieron. Motivo del dueño: las rocas pasaron a llamarse OBJETIVOS y su porcentaje **ya es** la coherencia, que se muestra aparte y ordena el ranking de célula — contando en los dos lados, un aprendiz que todavía no planificó una acción perdía puesto en el general por algo que el general no debería medir. Los dos pesos suman **1 exacto** (`0.75` + `0.25`): el 35% que dejaron las rocas se repartió entre los dos que quedaron, hábitos de `0.50` a `0.75` y cursos de `0.15` a `0.25`.
