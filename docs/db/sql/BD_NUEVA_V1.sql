...
COMMENT ON TABLE roles IS 'Catálogo RBAC. Los 5 roles del negocio como datos (antes: enum UserRole + matriz en código).';
...
COMMENT ON TABLE permisos IS 'Acción autorizable. La clave es la que usa @RequiresPermission en el backend.';
...
CREATE INDEX usuarios_rol_estado_idx ON usuarios (rol_id, estado);  -- listados de admin (el (role) suelto era redundante: P-29)
...
-- ── Perfiles: 1 tabla por rol, PK=FK (1:1 real, sin surrogate — P-28). ──────
...
CREATE TABLE perfiles_alquimista (
...
CREATE TABLE perfiles_lider_mentores (
    -- Tabla NUEVA: el rol existía sin perfil (P-04). Campos [PENDIENTE-CONFIRMAR].
    usuario_id     uuid PRIMARY KEY REFERENCES usuarios (id) ON DELETE CASCADE,
    bio            text,
    creado_en      timestamptz NOT NULL DEFAULT now(),
    actualizado_en timestamptz NOT NULL DEFAULT now()
...
--     IF (TG_TABLE_NAME = 'perfiles_mentor'         AND clave_rol <> 'MENTOR')
--     OR (TG_TABLE_NAME = 'perfiles_admin'          AND clave_rol <> 'ADMIN')
--     OR (TG_TABLE_NAME = 'perfiles_alquimista'     AND clave_rol <> 'ALQUIMISTA')
--     OR (TG_TABLE_NAME = 'perfiles_lider_mentores' AND clave_rol <> 'LIDER_MENTORES') THEN
