...
- **Hace DOS escrituras, en este orden** (mismo que el repo viejo, envueltas en una `@Transactional` local): (1) cierra el registro de HOY del hábito de catálogo `DAILY_CLASS` — puntos, racha y ventana de entrega, dominio de `habits`, vía `habits.api.CompletarClaseDiariaHabitoUseCase`; (2) marca la lección como vista en `leccion_progreso` (`CompletarLeccionUseCase`). **Ambos pasos son idempotentes ante un reintento genuino**: repetir el POST devuelve el resultado ya otorgado sin volver a sumar puntos (`EstadoRegistro.COMPLETADO` es terminal). La única excepción es un registro cerrado **sin** resumen guardado — ver el 409 de abajo.
...
- **409** `"Tu Clase Diaria de hoy quedo cerrada sin resumen..."` si el registro de hoy ya está `COMPLETADO` pero **sin resumen guardado**. Es un estado que ya no se puede producir (ver el punto siguiente); si aparece, es una fila anterior al arreglo de E-120. Se rechaza a propósito en vez de responder 200 descartando el texto en silencio, que es lo que hacía antes.
- **Sin este POST el hábito NO queda completado**: no hay estado intermedio "completado sin resumen". Si el aprendiz abandona el flujo a mitad de camino, su Clase Diaria sigue `PENDIENTE` y puede reintentarla desde Training.
  > **Corregido 2026-09-05 (E-120).** Esta línea decía la verdad del contrato y una mentira sobre el código: `DAILY_CLASS` era un `CHECKBOX` sin política propia, así que `POST /habit-tracks/{id}/complete` y la herramienta `marcar_habito_completado` del agente lo cerraban sin pedir resumen. Ahora `PoliticaClaseDiaria` cierra esas dos puertas — las dos responden **400** `"La Clase Diaria se da por cumplida al enviar el resumen de la clase, desde Training"` — y este endpoint es de verdad el único camino.
...
- **Notifications:** no se confirmó, solo con los archivos de este alcance, si algún módulo fuera de los 4 listeners de `notifications` invoca `EmitirNotificacionUseCase` para los 9 tipos de `TipoNotificacion` sin listener propio (`RECORDATORIO_HABITO`, `RECORDATORIO_ROCA`, `RECORDATORIO_RADAR`, `MENSAJE_MENTOR`, `ANUNCIO_SISTEMA`, `RESUMEN_SEMANAL`, `MENSAJE_CHAT`, `TICKET_RESPONDIDO`, `TICKET_ABIERTO`, `HABITO_PERSONAL_MODIFICADO`).
  > **Actualizado 2026-09-25 (D-168).** `RESUMEN_SEMANAL` dejó de estar sin listener: lo emiten los tres listeners del semáforo semanal.
