...
1. App llama `POST /support-tickets/attachments/upload-url` → recibe `{bucket, path, uploadUrl}` (`uploadUrl` firmada por `AlmacenamientoPort.firmarSubida`; desde el 2026-08-31 hay adaptador real, `S3AlmacenamientoAdapter` — se activa con `STORAGE_PROVEEDOR=s3` + credenciales resolubles por el SDK, ver D-54 en `docs/MODULOS_A_AVANZAR.md`. Sin esa propiedad sigue el default `NoOpAlmacenamientoAdapter`, que devuelve un placeholder).
...
| ~~D-S5~~ | ~~`AlmacenamientoPort` sigue siendo `NoOpAlmacenamientoAdapter` — las URLs firmadas son placeholders (`about:blank#...`)~~ ✅ **Resuelto 2026-08-31 (D-54).** Existe `S3AlmacenamientoAdapter`; el `NoOp` quedó como default explícito (`matchIfMissing=true`) para que el entorno local y los tests no necesiten credenciales | D-34 cerrado por D-54 | `shared/infrastructure/storage/S3AlmacenamientoAdapter.java`; `AlmacenamientoS3Config.java` |
