...
- **Contra quién se cuentan esos topes (corregido el 2026-09-21).** Los dos topes que *rechazan* una petición cuelgan ahora del **origen**: 20/hora por IP y 5/hora por la pareja (origen, correo). Antes el de 5/hora colgaba del correo a secas, y eso lo volvía una palanca contra su dueña: cinco peticiones anónimas con el correo de otra persona le cerraban la recuperación el resto de la hora — y otras diez contra `POST /auth/login`, que tenía el mismo defecto, le cerraban también la entrada. Quince peticiones por hora, sin cuenta y sin acertar ninguna contraseña, dejaban a una persona nombrada fuera de la aplicación. Queda **un solo contador colgado del correo**, `email-envios:`, con tope de 30/hora, y precisamente por eso **no rechaza nada**: cuando se agota, no sale más mail hacia esa casilla pero la petición responde 202 igual que siempre. Su trabajo es que nadie use nuestro servidor de correo para bombardear un buzón; no es una puerta que se pueda cerrar desde afuera. La regla general y el porqué del orden de las partes de la clave están en `OrigenDeLaPeticion`.
...
| A-5 | Umbrales de rate limit de reset de contraseña (§7.5): se asumieron 5/hora por la pareja (origen, correo) y 20/hora por IP sin confirmar con producto; el tope de envíos por casilla quedó en 30/hora (2026-09-21) | Nada hoy, son ajustables sin recompilar el patron (constantes en `ResetContrasenaService`) |
