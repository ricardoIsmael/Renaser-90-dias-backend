...
| 11 | `habitsAdmin.ts` completo sin backend (catálogo, horarios, guías/adjuntos) | 🟡 **Mayormente cerrado (2026-08-27).** CRUD admin completo de catálogo (`HabitoAdminController`), horarios (`HorarioHabitoAdminController`, PATCH real con distinción "ausente" vs "null explícito") y guías con `closePrevious` (`GuiaHabitoAdminController`) — todos `/api/v1/admin/habits/**`, solo ADMIN/ALCHEMIST. Adjuntos de guía IMAGEN/AUDIO: backend construido con el flujo de 2 pasos ya usado en `avatar`/`rocks`/`onboarding`/`calendar` (`POST .../guide-attachments/upload-url` → PUT directo → `POST .../confirm`, `SolicitarUrlAdjuntoGuiaUseCase`/`ConfirmarAdjuntoGuiaArchivoUseCase`), consistente con el 100% del resto del sistema y sin sumar capacidad nueva a `AlmacenamientoPort`. **Tensión real, no resuelta:** el frontend (`habitsAdmin.ts`) espera `multipart/form-data` de un solo POST — patrón que este backend no tiene en ningún lado (confirmado por grep, ni siquiera para portada de curso, que tiene la misma tensión). Requiere que el frontend adopte el patrón de 2 pasos (o, si el producto insiste en multipart, decidirlo explícitamente como excepción). `habitType`/`tipo` y `claveSistema` quedan protegidos como invariantes: no editables después de creado |
...
| 21 | Sin agregador `GET /home` (resumen del día) | 🟡 **Mayormente cerrado (2026-08-27).** `GET /api/v1/home` en `points` ahora devuelve `puntosLiga`/`coherencia`/`rachaActual`/`rachaMaxima` (dominio propio) + `diaPrograma`/`inscrito`/`fase` (`users.api.ParticipacionProgramaFinder`) + `habitosHoy{completados,total}` + `rocasHoy{completadas,total}` + `proximoEvento{eventoId,titulo,iniciaEn}` + `notificacionesNoLeidas`, con degradación a `null` documentada por widget si el finder correspondiente determina que no aplica (mismo criterio de "falla parcial" que ya usa el contrato móvil). **Hallazgo de arquitectura real durante la construcción:** los 4 finders nuevos (`habits`/`rocks`/`calendar`/`notifications`) no podían vivir en el módulo proveedor (`habits.api`, etc.) — esos módulos ya dependen de `points`/`users` para otorgar puntos o validar el actor, así que declarar el finder ahí y consumirlo desde `points`/`users` cerraba un ciclo real que Spring Modulith rechazó (`ArchitectureTest.modulesDoNotLeakInternals`). Se resolvió con el mismo patrón DIP que el código ya usaba para `points.api.PorcentajeRocasFinder`: la interfaz vive en el módulo consumidor (`points.api`/`users.api`), el proveedor la implementa. **Sigue en `bloqueos`:** `weekStatus`/`todayStatus`/`avatarState` (clasificación de cada día de la semana en 9 categorías + "mood" de avatar) — ningún módulo calcula ese algoritmo hoy, es una regla de negocio nueva sobre el historial semanal, no un simple finder de lectura; no se inventó (CLAUDE.MD §0.6). Nombres de campo siguen en español (continuidad con lo ya publicado); el remapeo a los nombres ingleses del contrato móvil actual es trabajo de Fase 1 |
| 22 | "Logros" no es un concepto de dominio en `points` | 🟡 **Mayormente cerrado (2026-08-27).** `GET /api/v1/profile/logros` construido en `users` (perfil), confirmado el contrato exacto contra el backend viejo (`service.ts:175-329`) y contra la app real (`C:\renaserPlayStore\src\types\logros.ts`). 8 de 9 campos resueltos: `programDay` (`users.api.ParticipacionProgramaFinder`), `totalHabitsCompleted`/`firstHabitCompletedAt` (`users.api.HabitoLogrosFinder`, implementado por `habits`), `totalRocksCompleted`/`firstRockCompletedAt`/`bestRocksStreakDays` (`users.api.RocaLogrosFinder`, implementado por `rocks` — el algoritmo de `bestRocksStreakDays` es 1:1 el `longestRocksStreakDays` del backend viejo), `radarEntriesCount`/`firstRadarEntryAt` (`users.api.RadarLogrosFinder`, implementado por `habits`). **Sigue sin cerrar: `streak`.** No es la mejor racha histórica de rocas (`bestRocksStreakDays`, campo distinto) — es días consecutivos hacia atrás desde ayer donde TODOS los hábitos del día estaban completos y, si había rocas asignadas, las 3 también (`computeStreak`, `service.ts:175-218`). Ningún finder actual expone esa lógica cruzada hábitos+rocas día-por-día; no se aproximó con un campo que signifique otra cosa (sería incorrecto). Devuelve `null` explícito y documentado — el tipo TS del móvil espera `number`, así que mostrar el valor hoy pintaría `"null"` en pantalla hasta que se decida quién construye ese finder cruzado. **Mismo hallazgo de arquitectura que el gap #21** (ciclo de módulos, resuelto con DIP: la interfaz vive en `users.api`, `habits`/`rocks` la implementan) |
...
| 25 | Panel admin de células/cohortes incompleto | ✅ **Cerrado 2026-08-27** (lo que dependía de esta sesión). `GET /admin/cells/dashboard` (cross-cohorte), `/mentores-disponibles`, `/mentores`, `/aprendices-disponibles` — ver docs/MODULO_COMMUNITY.md sec. 8. Asignar/quitar aprendiz de célula: `POST/DELETE /api/v1/admin/cells/{id}/trainees(/{traineeId})` en `community` (mismo namespace que `cells.ts` del frontend ya tenía escrito, aunque sin pantalla que lo usara todavía). `community` valida que la célula exista y que el destinatario sea un TRAINEE activo (dueño del agregado `Celula`); la escritura real de `participantes_programa.celula_id` se delega en `users.api.AsignacionCelulaPort` (nuevo, implementado por `ParticipacionProgramaService`) — `users` es dueño de la columna, no de la validación de asignación. Sigue sin resolver, fuera de alcance de este gap: `rankingPosition`/`coherenceScoreGroup` por célula (Q-1/Q-1b de `docs/MODULO_POINTS.md`, decisión de producto pendiente sobre quién puebla `ranking_celulas` y con qué fórmula) |
