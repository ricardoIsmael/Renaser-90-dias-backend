...
- ~~Rechazo si el hábito es `PERSONAL` (solo tiene sentido "elegir" hábitos de catálogo compartido — un hábito personal ya es exclusivamente del dueño que lo creó).~~ **Revertido el 2026-09-06 (E-138 / D-116).** El razonamiento era correcto para lo que la tabla significaba cuando se escribió esto; dejó de serlo cuando **D-87/V23** convirtió `desbloqueos_habito` en *"qué hábitos lleva este aprendiz en su plan"* y le puso encima el interruptor ACTIVO/PAUSADO. Con el rechazo puesto, ese interruptor devolvía **400** sobre todo hábito propio. Hoy se acepta el hábito `PERSONAL` **del propio actor**; el de otro aprendiz responde **404**.
...
- `src/test/java/com/renaser/os/habits/application/services/DesbloqueoHabitoServiceTest.java` — se actualizó el constructor existente (4 dependencias nuevas) y se agregaron: día 0 rechazado, hábito personal rechazado, hábito inactivo rechazado, elección normal devuelve el desbloqueo asegurado, elegir dos veces seguidas es idempotente (mismo resultado, sin excepción). — **Dos de esos tests ya no existen con esa forma:** "día 0 rechazado" se dio vuelta con **D-103** (hoy el día 0 desbloquea para el día 1) y "hábito personal rechazado" con **E-138 / D-116** (hoy el hábito propio se acepta y el ajeno da 404). Los métodos no se borraron: se invirtieron, para que quede a la vista qué regla cambió.
