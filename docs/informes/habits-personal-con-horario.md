...
- ~~**Participante en `dia_programa = 0` (programa no activado):** el alta de hábito personal falla con `400` y un mensaje que menciona `diaInicio`…~~ — **CERRADO el 2026-09-06 (D-115, `BITACORA_ERRORES.md` E-137).** Se confirmó en producción que no era un caso de borde: por D-66 el Día 1 nunca puede ser hoy, así que **todo** aprendiz recién aprobado pasa su primera jornada en `dia_programa = 0` y el alta se caía con `IllegalArgumentException: diaInicio fuera de rango 1..90: 0` para el 100% de las cuentas nuevas. La decisión de negocio que faltaba ya existía: **D-103** (2026-09-04) eligió `Math.max(1, diaActual)` para la operación hermana `DesbloqueoHabitoService.resolverDiaDesbloqueo`, y se aplicó la misma aquí — lo que se crea en día 0 arranca el día 1. Cubierto ahora por `enDia0ElHabitoPersonalSeCreaYSuHorarioArrancaElDia1` y `enDia0ElHabitoPersonalSeCreaIgualConElRelojEnLaMadrugadaUtc`, los dos rojos contra el código anterior.
