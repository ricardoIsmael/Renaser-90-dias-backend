...
| DN-6 | `PATRON_DE_MALESTAR_REPETIDO` (V59, 2026-09-15) es un valor más de `tipo_notificacion` y le llega a **todos** los ADMIN/ALCHEMIST activos, resueltos por `ParticipacionProgramaFinder.usuariosActivosConRol` | Tercera vez que se toma la misma decisión (V46 `ACOMPANAMIENTO_ALUMNO`, V49 `GRUPO_POR_VENCER`) y por el mismo motivo: la bandeja, las preferencias, la deduplicación por `origen_evento_id` y la purga a 90 días ya existen y se reutilizan tal cual. Va a todos porque no hay un "dueño" del caso, igual que con los grupos por vencer. **Dos restricciones propias de este tipo**, que no aplican a ningún otro: el texto **no puede afirmar nada sobre la persona** (dice que se repitió un patrón que conviene mirar — no "crisis", no "riesgo") y **nunca lleva una palabra de lo que la persona escribió**, tampoco en el push, que sale con el mismo cuerpo. `PatronDeMalestarNotificationListenerTest` afirma las dos cosas. Origen del evento: `rag.api.PatronDeMalestarRepetidoEvent` (D-143 en `docs/MODULO_RAG.md`) |
...
> **Actualizado 2026-09-25 (D-168).** Se suman tres listeners del semáforo de cumplimiento del aprendiz, todos con el tipo `RESUMEN_SEMANAL` (existía desde V1 sin emisor): `SemanaDelSemaforoCerradaNotificationListener` (a la persona), `ResumenSemanalDelGrupoNotificationListener` (al mentor) y `ResumenSemanalGeneralNotificationListener` (a cada MENTOR_LEAD, ADMIN y ALCHEMIST activo). Título y cuerpo **sin cifras ni nombres de personas**, porque `NotificacionService.intentarPush` manda los mismos textos en el push. El texto cambia según el caso (`notifications.domain.model.semaforo.RedaccionDelSemaforo`: con o sin registros, aprendices que necesitan apoyo, nadie con alertas, neutro), sin cifras ni colores; lo decidió el dueño el 2026-09-25 en vez de aprobar textos fijos.
>
