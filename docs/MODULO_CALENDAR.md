...
- **`RecordatorioServiceTest.despacharPublicaUnEventoPorRecordatorioVencido` publicaba un `eventoId` distinto al esperado.** No era un bug de `RecordatorioService` — el servicio arma el evento de dominio con `evento.id().value()`, que en producción SIEMPRE coincide con el id por el que se consultó (`loadEventoPort.byId(recordatorio.eventoId())` devuelve la fila con ESE id). El bug era del escenario del test: el helper `evento(TipoEvento tipo)` usaba `Evento.crear(...)`, que SIEMPRE asigna un `EventoId.newId()` al azar — el mock `loadEventoPort.byId(eventoId)` quedaba devolviendo un `Evento` con un id distinto al `eventoId` que el test declaraba y esperaba de vuelta. Corregido: el helper ahora usa `Evento.rehydrate(eventoId, ...)`, fijando el id exacto — igual que devolvería el adaptador de persistencia real. Sin cambios en `RecordatorioService`.
  > **Actualizado 2026-08-31 — el workaround ya no hace falta.** Al migrar `calendar` al puerto `IdGenerator` (CLAUDE.MD §5.4.7: `domain/` sin aleatoriedad), `EventoId.newId()` desapareció y `Evento.crear(...)` pasó a **recibir** el `EventoId` como primer parámetro. El helper volvió a `Evento.crear(eventoId, ...)`: fija el id igual de bien, pero ahora pasa por las validaciones reales del agregado en vez de saltearlas con `rehydrate`, que era el precio que se pagaba por fijar el id. Este es el beneficio concreto de sacar la generación de identidad del dominio: la factoría es referencialmente transparente y el test puede comparar por igualdad en vez de esquivar el id.
...
Alcance: solo lectura sobre `src/main/java/com/renaser/os/calendar/`, sin correr `./mvnw`. Comparado contra CLAUDE.MD §5.1/§5.1.2/§5.4.

### Hallazgos

1. **Endpoints de `EventoController` sin `@RequiresPermission`/`@PublicEndpoint`** — `src/main/java/com/renaser/os/calendar/infrastructure/adapter/in/rest/evento/EventoController.java:52-189`. La anotación ya existe y se usa en el repo (`src/main/java/com/renaser/os/users/infrastructure/adapter/in/rest/accountrequest/AccountRequestController.java`), pero `calendar` no la adoptó en ninguno de los 9 endpoints. El test de reflexión que CLAUDE.MD §0.3/§12 exige ("falla si un endpoint no declara `@RequiresPermission` ni `@PublicEndpoint`") no puede existir todavía para este módulo sin que falle. Autorización real hoy vive en `AccesoEventoService`/`EventoService` (guard clauses de rol + `NotAuthorizedException`), que es correcto como *mecanismo*, pero no está declarada de forma auditable a nivel de endpoint.

2. **`RecordatorioService.generarParaEvento` supera el techo de 40 líneas** — `src/main/java/com/renaser/os/calendar/application/services/RecordatorioService.java:112-160` (49 líneas). Concentra: expansión de ocurrencias, resolución de destinatarios, anuncio, cálculo de instantes de recordatorio por ocurrencia y encolado — varias responsabilidades en un único método privado. Candidato a extraer, p. ej., el bloque de las líneas 137-158 (recordatorios por ocurrencia) a un método con nombre propio (`encolarRecordatoriosDeOcurrencia(...)`).

3. **`EventoService` agrupa 8 casos de uso en una sola clase** (`ListarEventosParaVisorUseCase`, `ObtenerEventoUseCase`, `CrearEventoUseCase`, `ActualizarEventoUseCase`, `EliminarEventoUseCase`, `CancelarOcurrenciaUseCase`, `SolicitarUrlPortadaUseCase`, `ConfirmarPortadaUseCase`) — `src/main/java/com/renaser/os/calendar/application/services/EventoService.java:48-51`. 340 líneas de archivo / ~293 de clase, todavía bajo el techo duro de 300 pero por encima del objetivo de 150, y en tensión directa con CLAUDE.MD §5.4.8 ("una clase por caso de uso, no un `Service` con 30 métodos"). No es una violación de línea de código sino de SRP: los 8 casos de uso comparten mucha lógica de autorización (`requireRolCreador`, `requirePropioSiMentor`), que ya está bien extraída — pero eso mismo sugiere que se podría partir en clases más chicas que reciban esos helpers compartidos (o promoverlos a un colaborador, similar a como ya se hizo con `AccesoEventoService`) en vez de una clase que implementa 8 interfaces `port/in`.

4. **`domain/model/evento/` tiene 16 clases sueltas**, por encima de la señal de alarma de ~10 (CLAUDE.MD §5.1.2): `EstadoEvento`, `Evento`, `EventoId`, `Excepcion`, `ExpansorOcurrencias`, `FrecuenciaRecurrencia`, `Ocurrencia`, `Recurrencia`, `ReglaRecordatorio`, `ReglasPorTipoEvento`, `ResolverAudiencia`, `RolUsuario`, `TipoAudiencia`, `TipoEvento`, `TipoReglaRecordatorio`, `TipoUbicacion`. Aplicando la regla corregida de §5.1.2 (subcarpeta = agregado, no capa), la mayoría de estas clases sí son legítimamente parte de un solo agregado (`Evento` + sus value objects/enums de recurrencia y ocurrencia) y no ameritan partirse. La excepción real es `RolUsuario` (y en menor medida `TipoAudiencia`): se usan también fuera del agregado `Evento` — en `application/services/AccesoEventoService.java`, `application/ports/out/participante/ConsultarProgresoParticipanteCalendarPort.java`, `infrastructure/adapter/out/persistence/participante/RolUsuarioSqlMapper.java` — es decir, son vocabulario compartido del módulo (rol del participante), no un detalle del agregado `Evento`. Vale evaluar si `RolUsuario` debería vivir en un lugar más neutral (p. ej. junto a `nivelmembresia`/`recordatorio`, o en la raíz de `domain/model/`) en vez de en `domain/model/evento/`. No es una violación de la regla de subcarpetas (sigue siendo plano, sin subdividir por capa), es una observación de ubicación.

5. **Sin MapStruct en todo el módulo** — `grep -rl org.mapstruct` no encontró nada bajo `calendar/`. `EventoPersistenceMapper.java` (146 líneas) mapea `EventoJpaEntity ↔ Evento` completamente a mano. CLAUDE.MD §5.4.5/§6 fija MapStruct como la herramienta decidida para esa frontera específica en los 14 módulos. No es un error funcional (el mapeo a mano es más verboso pero igual de seguro), pero es una inconsistencia con la convención documentada — vale una decisión explícita: ¿se porta `calendar` a MapStruct en esa frontera, o se deja constancia de que el mapeo multi-tabla del agregado (`eventos` + `recurrencias_evento` + `dias_semana_recurrencia` + `roles_destino_evento` + `reglas_recordatorio_evento`) es un caso legítimo de excepción?

### Verificado sin hallazgos

- **`domain/` no importa `org.springframework.*` ni `jakarta.persistence.*`** en ningún archivo del módulo (`grep` limpio).
- **`domain/` no loguea** — sin `Logger`/`log.` en ningún archivo de `domain/`.
- **Lombok en `domain/`** limitado al subconjunto permitido: `Evento.java:26-29` y `RecordatorioEvento.java:24-27` usan solo `@Getter @Accessors(fluent = true) @AllArgsConstructor(access = AccessLevel.PRIVATE) @EqualsAndHashCode(of = "id")`. Sin `@Data`/`@Setter`/`@NoArgsConstructor` público en ningún archivo de dominio.
- **Nombres prohibidos** (`Util`/`Helper`/`Manager`/`Processor`/`Data`/`Info` sueltos): ninguno encontrado (los únicos matches de `find -iname "*Info.java"` son los dos `package-info.java`, que es la convención estándar de Java, no una clase de negocio).
- **`domain/model/` sí está correctamente separado por agregado**: `evento/`, `confirmacion/` (`Confirmacion`, `EstadoConfirmacion`), `nivelmembresia/` (`NivelMembresia`, `ProgresoNivel`) y `recordatorio/` (`RecordatorioEvento`, `CalculadoraRecordatorios`, `InstanteRecordatorio`) son 4 agregados independientes, cada uno con su propio puerto/repositorio — cumple la regla corregida de §5.1.2.
- **`EventoController` es "tonto" por endpoint**: los 9 métodos de ruta miden entre 3 y 4 líneas cada uno, ninguno inyecta un puerto `out` ni un repositorio, ninguno tiene `@Transactional` ni un `if` de regla de negocio. Sí concentra ~40 líneas de mapeo wire→comando en métodos privados (`aComandoCrear`/`aComandoActualizar`/`recurrenciaDe`/`rolesDestinoDe`/`reglasDe`) — es "Full Mapping" a mano tal como exige §5.4.1/§5.4.5 para la frontera web, no lógica de negocio, así que no viola la regla de controller tonto, pero es la parte más larga del archivo.
- **Ningún otro método relevado supera 40 líneas** salvo el de hallazgo 2 (`EventoService.actualizar` con 36 líneas es el segundo más largo, dentro del techo).
- **Ningún archivo del módulo supera 300 líneas** (`EventoService.java` con 340 líneas de archivo es el más grande, pero cuenta imports/comentarios; el cuerpo de clase real es ~293 líneas).
