...
- **H-6 — `TipoDia` resuelto solo como DOMINGO/DISCIPLINA por día de calendario; la intoxicación es una regla de exigencia, no un tipo de día (D-169).** Los Ciclos de Intoxicación Consciente —días 8-10 (VER), 17-19 (CORTAR) y 26-28 (RENASER), de la especificación del cliente, confirmados por el dueño el 2026-09-25— viven en `CicloIntoxicacion` y los aplica `Habito.esOpcionalEnDia(diaPrograma)`: en esos días cada registro nace con `es_opcional = true` salvo los hábitos `obligatorio_en_intoxicacion` (POST DIARIO EN COMUNIDAD, ya marcado en V4). El día de programa es el que `users` deriva de las fechas en la zona de la persona, con `dias_ajuste_programa`. `TipoDia.INTOXICACION` sigue sin asignarse a propósito: el tipo del día decide qué horario rige y los registros lo reusan para volver a encontrarlo (ventana de puntos, avisos, Santuario); un día de intoxicación conserva su horario de siempre.
  > **Corregido 2026-09-25 (D-169).** Decía: «`INTOXICACION` (ciclos fijos del programa) no se implementó — el paso 0 no alcanzó a extraer `INTOXICATION_CYCLES` con precisión suficiente para no inventarlo. `TipoDia.INTOXICACION` existe en el enum (fiel al baseline) pero ningún caso de uso lo asigna todavía.» Los ciclos los trajo la especificación del cliente (`docs/CUMPLIMIENTO_REQUISITOS.md` §2), no el código viejo.
...
2. ~~**¿Cuáles son los ciclos exactos de `INTOXICACION`?**~~ **Resuelta e implementada (D-169, 2026-09-25):** días 8-10, 17-19 y 26-28, todos los hábitos opcionales salvo la publicación diaria en comunidad (ver H-6). Quedan abiertas cuatro preguntas derivadas, registradas en D-169: si la bandera `obligatorio_en_intoxicacion` vuelve obligatorio a un hábito opcional de catálogo (se asumió que no), si los hábitos personales también se aflojan (se aplicó «todos», como dice la especificación), qué significan los horarios DISCIPLINA/INTOXICACION en esos días, y si se corrigen los registros de esos días generados antes del cambio.
   > **Corregido 2026-09-25.** Decía: «No se encontraron en el paso 0 con la precisión necesaria para implementarlos sin inventar. `TipoDia.INTOXICACION` queda sin ningún caso de uso que lo asigne.»
