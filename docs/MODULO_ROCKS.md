...
- **Hueco #17 (`esPrincipal`/`publishedToWall` al completar evidencia): CERRADO COMPLETO 2026-08-26 (noche).** `esPrincipal` ya estaba construido; `publishedToWall` se cerró desde el lado de `community` — ver `docs/MODULO_COMMUNITY.md` §8.2 (`community.api.PublicarEnMuroPort`, "Camino B" que §11.2 dejaba planteado). Único archivo de `rocks` reabierto: `RocaDiariaService.java` (+ el comando/DTO/controller de completar evidencia). Ver §11 actualizado.
...
│   ├── rocasemanal/    RocaSemanal, RocaSemanalId, EstadoPlazo,
│   │                   VentanaPlanificacionSemanal, SemanaPrograma
│   │                   (AccionCritica se borro el 2026-09-23 con la tabla — RK-14)
...
| RK-9 | **`rocks` pasó a CREAR y CORREGIR Rocas Maestras (2026-09-07).** Hasta acá el javadoc del agregado decía que no lo hacía —que las creaba `onboarding`, "Ola 5, no construido todavía"— y §6 lo listaba como pendiente. Dos cosas dejaron de ser ciertas: `onboarding` ya existe y **nunca las creó**, así que `rocas_maestras` estaba **vacía en producción** (verificado: 0 filas con 2 participantes) y la pantalla de Objetivos del Plan mostraba un objetivo inventado escrito a mano en el frontend, igual para todos los aprendices. Y el aprendiz corrige su objetivo **durante** el programa, no solo al arrancar: eso no es onboarding, es gestión de la propia meta, y vive donde vive el agregado. Si algún día `onboarding` necesita sembrarlas, el camino es un puerto en `rocks.api`, no un INSERT desde otro módulo. `PUT /rocks/master/{eje}` es un upsert por eje, idempotente por diseño porque la tabla ya impone `UNIQUE (participante_id, eje)`. Migración `V35`: se agregaron `meta`, `avance`, `unidad` y `actualizado_en` — el diseño mostraba una barra de avance ("Llevas 19.500 de 30.000 USD") que no tenía dónde vivir. Las tres primeras van juntas o ninguna, por CHECK: un objetivo puede ser cualitativo, pero media meta no significa nada. |
| RK-10 | **Nivel MENSUAL (2026-09-07).** El cliente definió el plan en cuatro niveles: tres objetivos maestros a 90 días, tres mensuales, tres semanales y tres actividades diarias. El backend ya cumplía tres de los cuatro sin que nadie lo hubiera notado: `rocas_maestras` es una por eje, `CrearPlanSemanalUseCase` exige exactamente 3 (`@Size(min=3,max=3)`, una por eje) y `BloqueoPlanificacion.ROCAS_REQUERIDAS_MANANA` pide 3 por día. **El mensual no existía**, así que se saltaba del objetivo de 90 días directo a la semana. `V36` crea `rocas_mensuales`. **Cuelga de la Roca Maestra y no del participante**, igual que la semanal: así "tres mensuales" sale de la misma regla que ya da tres semanales —tres maestras, una mensual por maestra y por mes— en vez de exigir un contador aparte que se pueda desincronizar. **El mes cuenta 30 días, no cuatro semanas**: cuatro semanas dan 28, y con eso el mes 3 cerraría el día 84 y seis días del programa quedarían fuera de todo mes; de a 30 los tres cubren los 90 exactos, que es como lo pidió el cliente (30/60/90). La contra, asumida: el corte no cae siempre en domingo, a diferencia de la semana de programa que calcula `SemanaPrograma` de lunes a domingo. Son dos conceptos distintos y `MesPrograma` lo dice en su javadoc para que nadie los mezcle. El comando recibe **el eje, no el `RocaMaestraId`**: aceptar ese UUID desde afuera abriría la puerta a mandar el de otra persona. Y si no existe la maestra de ese eje, definir un mensual falla con un mensaje que pide definir primero el objetivo de 90 días — un tramo sin objetivo mayor no significa nada. |
| RK-11 | **El objetivo del mes se calcula solo (2026-09-22).** RK-10 dejó dónde guardar el tramo mensual, pero el número lo tenía que inventar la persona, y el frontend había terminado calculándolo por su cuenta con una fórmula distinta a la de los hitos del Mapa — dos cifras para el mismo mes (E-203). Ahora lo calcula `CalculadoraObjetivoMensual` sobre `CurvaDelPlan` (40/75/100 %, §3 V08 del manual), anclado en el valor REAL de hoy: ver D-147 para la fórmula y por qué unifica las dos. **Tres piezas que viven en lugares distintos**: la Roca Maestra (este módulo), el día de programa (`ConsultarProgresoParticipanteRocksPort`) y **qué se mide**, que está en las respuestas del Mapa y llega por el puerto nuevo `onboarding.api.MedicionDelMapaFinder` — no por SQL contra `respuestas_onboarding`, que sería una tabla ajena (D-41). **Editable sin endpoint nuevo**: el `PUT` de RK-10 ya existía, y una fila guardada en `rocas_mensuales` manda sobre el cálculo (`editado: true` en la respuesta). **Limitación conocida, heredada:** una cifra mensual de **0** no se puede guardar —`CHECK meta > 0` en V36, y `MetaCuantitativa` exige punto de partida para una meta de cero, columna que `rocas_mensuales` no tiene—. Toca a quien quiera saldar una deuda entera dentro de un mes; el cálculo sí la produce, solo no se puede fijar a mano. |
| RK-12 | **La semana se abre con UN objetivo, no con tres (2026-09-22).** `CrearPlanSemanalCommand` exigia `@Size(min=3,max=3)`: para guardar la semana habia que llenar los tres ejes, o sea doce campos minimos de una sentada, o no guardar nada. Pedido del dueno tras el reporte de los aprendices. Ahora es `min=1`: manda el eje que la persona eligio como principal en el Mapa y los otros dos se suman cuando quiera. **El maximo sigue en tres** — hay un objetivo semanal por eje y no existe un cuarto. **Completado el 2026-09-23 (RK-15):** faltaba poder sumar los otros dos despues; hasta ese dia guardar el principal los bloqueaba toda la semana. **Lo que NO se puede olvidar al tocar esto** (y es el motivo de que sea una entrada y no una linea): `DashboardRocasService` calculaba `tieneRocaSemanal = semanalesVista.size() >= EjeObjetivo.values().length`. Sin cambiar eso, la persona planificaba su eje principal y el dashboard le seguia diciendo *"todavia no armaste esta semana"* — el cambio de validacion sin el del dashboard es peor que no haber cambiado nada. El dashboard pregunta si la semana esta ARMADA, no si esta completa. ~~**Sin tocar:** cada objetivo semanal sigue exigiendo exactamente 3 acciones criticas (`RocaSemanal.requireAccionesValidas`)...~~ **Corregido el 2026-09-22 (RK-13) y el 2026-09-23 (RK-14).** Duro horas: ese mismo dia las acciones bajaron al objetivo diario, y al dia siguiente `AccionCritica` y su tabla dejaron de existir. El objetivo semanal hoy es un titulo. El prellenado desde el Mapa (`acciones_mapa`) sigue vivo, pero alimenta el planificador del **dia**, no el de la semana. |
| RK-13 | **Las acciones pasan del objetivo semanal al DIARIO (2026-09-22, V61).** El dueno describio la cadena del plan asi: *"objetivo de los 90 dias, luego mensual, luego semanal, y luego objetivo diario, y estos objetivos diarios tienen acciones para hacerlo"*. Los cuatro niveles ya existian; lo que estaba en el lugar equivocado eran las acciones. `acciones_criticas` colgaba de `rocas_semanales`, o sea que el domingo habia que escribir nueve acciones —tres por eje— sin saber todavia que dia se iban a hacer, y despues volver a agendarlas una por una. Ahora `acciones_diarias` cuelga de `rocas_diarias` y se escriben al planificar el dia. **Cambia la cardinalidad, no solo el padre:** las criticas de la semana eran exactamente 3 y obligatorias; las del dia son **0 a 3**, porque un objetivo puede ser una sola cosa que no necesita desglose y obligar a inventar tres es lo que llevaba a escribir relleno. ~~**`acciones_criticas` NO se borra**: no se puede verificar desde el entorno local que este vacia en produccion...~~ **Corregido el 2026-09-23 (RK-14):** el dueno lo verifico y la V62 la borro. `RocaSemanal` ya no acepta acciones. **Tampoco llevan `completada`**: el objetivo del dia se cierra por evidencia y solo por evidencia (R-02), y un tilde por accion abriria un segundo camino para decir "esto ya esta" sin evidencia detras. |
| RK-14 | **Se borra `acciones_criticas`, y se deja el campo del JSON (2026-09-23, V62).** La RK-13 la habia dejado viva por una razon honesta —*"no se puede verificar desde aca que este vacia en produccion"*—. El dueno lo verifico (*"ya confirme que esta vacia"*) y aca se va: la tabla, `AccionCritica`, `AccionCriticaEmbeddable`, el `@ElementCollection` de `RocaSemanalJpaEntity`, `accionCritica1/2/3` del alta y `accionesCriticas` de la edicion. El objetivo semanal quedo en lo que es: un titulo, un obstaculo, una contingencia y una autoevaluacion. **La migracion no es un `DROP` pelado:** cuenta primero y solo borra si esta vacia — la confirmacion de que lo estaba vino de una persona mirando una base, no de algo que la migracion pudiera comprobar sola. ~~Aborta si encuentra filas.~~ **Corregido el 2026-09-23 (E-210):** abortaba, y eso tiro produccion — Flyway corre al arrancar. Ahora deja un WARNING y sigue, tambien si el `DROP` se rechaza por permisos (`42501`), que es lo que no se podia ver probando como `postgres`. **Lo que NO se borro, y es el punto de esta entrada:** `accionesCriticas` sigue viajando en `RocaSemanalResponse` como `[]`. La app **no tiene actualizacion por aire** (no usa `expo-updates`): quien no reinstale corre un esquema donde ese campo es obligatorio, y sacarlo del JSON lo dejaria sin ver su plan semanal. Se va cuando el padron este en un build posterior al `698a276`; `RocaSemanalResponseTest` esta ahi para que nadie lo "limpie" antes. **Borrar una tabla se deshace con otra migracion; sacar un campo de una respuesta rompe telefonos ya instalados.** | 
| RK-15 | **La semana se rechaza por EJE, no por semana (2026-09-23).** RK-12 bajo el minimo a un eje para que nadie tuviera que llenar los tres de una sentada. Quedo a medias: `crear` seguia haciendo `ALREADY_PLANNED` si la semana tenia **cualquier** objetivo, asi que al guardar el principal los otros dos quedaban bloqueados **hasta el domingo siguiente**. En pantalla se veia peor de lo que suena: el dueno abria Negocio, la tarjeta *TU SEMANA* le mostraba el objetivo de **Cuerpo** —porque listaba los ejes que tenian roca, sin filtrar por el que estaba mirando— y no habia ningun boton para planificar el suyo. *"En negocio no deja planificar, igual el de relaciones"*. **Lo que cambio:** `crear` calcula que ejes ya tienen objetivo esta semana y crea solo los que faltan; `ALREADY_PLANNED` queda para cuando **todos** los pedidos ya existen. Un objetivo guardado **no se pisa** desde ahi — para cambiarlo esta `editar`, que es donde vive la ventana de rectificacion. En la app, cada eje sin objetivo se dibuja con su boton *Planificar X*, y el asistente ya no vuelve a pedir los ejes que ya tienen. **La leccion:** bajar un minimo no es lo mismo que permitir completarlo despues. Sin la segunda mitad, "llena solo el principal" no significa "los otros cuando quieras" sino "perdiste la semana". | 
...
- ~~**`publishedToWall` al completar una Roca con evidencia (Hueco #17, 2026-08-26)** — no construido, faltan puertos públicos en `evidence`/`community`.~~ **Construido 2026-08-26 (noche)** — `community.api.PublicarEnMuroPort`, ver §11 y `docs/MODULO_COMMUNITY.md` §8.2.
- ~~**Creación/actualización de Rocas Maestras** — las crea `onboarding` (Ola 5, no existe). `rocks` solo lee.~~ **Cerrado 2026-09-07 (RK-9)**: `onboarding` ya existe y nunca las creó, así que la tabla quedó vacía en producción. La escritura vive ahora en `rocks`, dueño del agregado, con `PUT /rocks/master/{eje}`.
...
| Integración Testcontainers | `ConsultarProgresoParticipanteRocksPersistenceAdapterTest`, `RocaMaestraPersistenceAdapterTest`, `RocaSemanalPersistenceAdapterTest` (desde el 2026-09-23 cubre el riesgo inverso: que **no** quede nada apuntando a `acciones_criticas`, ya borrada), `RocaDiariaPersistenceAdapterTest`, `EventoVerdugoPersistenceAdapterTest`, `RegistrarEvidenciaRocaPersistenceAdapterTest` (confirma el INSERT nativo contra `evidencias` real) |
...
- ~~El `@ElementCollection` de `AccionCriticaEmbeddable` sobre `acciones_criticas`~~ **Cerrado el 2026-09-23 (RK-14):** el mapeo y la tabla se borraron. El unico `@ElementCollection` que queda en `rocks` es el de `acciones_diarias` (V61).
...
**Encargo:** aceptar `publishedToWall` y `esPrincipal` al completar una Roca con evidencia (`POST /api/v1/rocks/{id}/evidence`). **Ambos construidos completos, 2026-08-26.** `esPrincipal` se cerró primero (§11.1); `publishedToWall` se cerró después, del lado de `community` (§11.2, actualizado — el "Camino B" que esta sección dejaba planteado ya existe).
...
### 11.2 `publishedToWall` — CERRADO 2026-08-26 (noche), "Camino B" (publicación real)

Se implementó el Camino B que esta sección dejaba planteado, no el Camino A (solo el flag): `evidencias.publicada_en_muro` sigue sin tocarse (columna que `evidence` documenta explícitamente fuera de su alcance, `docs/MODULO_EVIDENCE.md:127`) — en cambio, se creó una publicación REAL en `publicaciones_muro`, que es lo que de verdad hace que la evidencia aparezca en el feed del Muro.

- **`community.api.PublicarEnMuroPort`** (nuevo, construido por el agente de `community` en paralelo con este) — `publicarDesdeEvidencia(autorId, texto, bucket, ruta, mime)` crea una `Publicacion` con `tipo=HITO_AUTOMATICO` (el valor del enum que ya existía sin ningún generador real, CM-7 de `docs/MODULO_COMMUNITY.md`) dentro de la misma transacción del llamador. Ver `docs/MODULO_COMMUNITY.md` §8.2 para el detalle completo del lado de `community`.
- **`CompletarRocaDiariaUseCase.CompletarRocaDiariaCommand`** — nuevo campo `boolean publishedToWall`. A diferencia de `esPrincipal` (default `true` por compatibilidad), este default es `false`: es funcionalidad nueva sin comportamiento previo que preservar. Rechazado con `IllegalArgumentException` (400) en el constructor compacto del comando si `publishedToWall=true` y el `tipo` de evidencia no es visual (`FOTO`/`VIDEO`/`CAPTURA`) — `TEXTO`/`AUDIO` no tienen archivo que mostrar en un carrusel.
- **`RocaDiariaService.completar()`** — si `command.publishedToWall()`, después de completar la roca y pagar el premio (mismo `@Transactional`), llama al nuevo puerto con: `texto = "Complete mi Roca: " + roca.titulo()` (no existe un campo de "leyenda" propio en el flujo de evidencia de rocas, así que se usa el título ya existente en vez de inventar un campo nuevo de request) y `mime` inferido del tipo de evidencia (`FOTO`/`CAPTURA` → `image/jpeg`, `VIDEO` → `video/mp4` — el comando de evidencia de rocas no transporta un mime propio).
- **`CompletarRocaDiariaRequest`/`RocaDiariaController`** — nuevo campo `Boolean publishedToWall` (nullable, `null` → `false`).
- **Único archivo de `rocks` reabierto además de estos tres**: ninguno — el cambio se contuvo exactamente al flujo de completar evidencia, como pedía el encargo ("único archivo que deberías tocar").

**Nota de seguridad, ya aplicada:** el chequeo de propiedad de la roca (`requireRocaPropia`) sigue corriendo ANTES de llegar a `publicarEnMuro` — el actor que publica es siempre `command.actorId()`, nunca un id que viaje suelto desde otro lado, así que no hay forma de publicar en el Muro a nombre de otro participante a través de este flujo (CLAUDE.md §0.3/E-42).

Pruebas nuevas en `RocaDiariaServiceTest`: `publishedToWallConEvidenciaNoVisualEsRechazado`, `publishedToWallLlamaAlPuertoDeCommunity`, `publishedToWallFalseNoLlamaANadie`.

---

## 12. Hueco #18 — `DestinoVerdugo`: hábitos personales confirmados, Código Renaser sigue bloqueado — investigado 2026-08-26

**Encargo (del lado de `habits`, territorio de `rocks`):** confirmar si `DestinoVerdugo` acepta hábitos personales por FK, y agregar un valor para "Código Renaser" si hace falta. Investigación de solo lectura — **ningún archivo de `rocks` se modificó**, salvo un test nuevo que prueba (no solo documenta) el primer punto.

- **Hábitos personales: YA funcionan, sin cambio de código, confirmado contra la base real.** `DestinoVerdugo.REGISTRO_HABITO` guarda `destinoId` = `registros_habito.id` (arco exclusivo, RK-8). Como `habits` unificó `habits`+`personal_habits` en una sola tabla `habitos` discriminada por `ambito` (P-12), `registros_habito.habito_id` referencia esa tabla unificada sin importar si el hábito es SISTEMA o PERSONAL — y `VerificarDestinoVerdugoPersistenceAdapter.registroHabitoPerteneceA` (`SELECT COUNT(*) FROM renaser.registros_habito WHERE id = ? AND participante_id = ?`) nunca joinea `habitos.ambito`. Un `EventoVerdugo` contra el track de un hábito PERSONAL pasa `requireDestinoPropio` exactamente igual que uno de un hábito SISTEMA. Nuevo test `VerificarDestinoVerdugoPersistenceAdapterTest` (Testcontainers) prueba los tres casos: hábito SISTEMA propio, hábito PERSONAL propio, hábito PERSONAL ajeno (rechazado).
- **Código Renaser (`RADAR`): sigue sin valor en el enum — no se agregó, y no es solo un tema de esquema.** Dos bloqueantes reales:
